        ...
        .directive('wysiwyg', function($timeout, wysiwgGui, $compile) {
            return {
                template: '<div>' +
                    ...
                    '</style>' +
                    '<div class="wysiwyg-menu" style="margin: 5px 0px;"></div>' +
                    '<div id="{{textareaId}}" ng-show="{{hideOptions}}" style="min-height: {{textareaHeight}}; max-height: 100px; width: 100% !important; overflow-x: hidden;color: #1d1d26;font-family: tahoma;" contentEditable="{{!disabled}}" class="{{textareaClass}} wysiwyg-textarea" rows="{{textareaRows}}" name="{{textareaName}}" required="{{textareaRequired}}" placeholder="{{textareaPlaceholder}}" ng-model="value"></div>' +
                    '</div>',
                restrict: 'EA',
                scope: {
                    value: '=ngModel',
