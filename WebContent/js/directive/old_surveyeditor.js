...
(function(angular, undefined) {
    ...
    angular.module('wysiwyg.module', ['colorpicker.module'])
        .directive('wysiwyg', function($timeout, wysiwgGui, $compile) {
            return {
                template: '<div>' +
                    ...
                    '	.editorBtn { margin: 3px; }' +
                    '   .wysiwyg-colorpicker { font-family: arial, sans-serif !important;font-size:13px !important; padding:2px 6px !important;}' +
                    '</style>' +
                    '<div class="wysiwyg-menu" ng-show="editor" style="margin: 5px;"></div>' +
                    '<div id="{{textareaId}}"  style="min-height: {{textareaHeight}}; overflow-y: scroll; max-height: 100px; width: 85%; overflow-x: hidden;" contentEditable="{{!disabled}}" class="{{textareaClass}} wysiwyg-textarea" rows="{{textareaRows}}" name="{{textareaName}}" required="{{textareaRequired}}" placeholder="{{textareaPlaceholder}}" ng-model="value"></div>' +
                    '</div>',/*ng-click="showEditor()" ng-dblclick="hideEditor()"*/
                restrict: 'EA',
                scope: {
                    value: '=ngModel',
        ...
        .value('wysiwgGuiElements', {
            ...
            'font-color': {
                tag: 'button',
                classes: 'wysiwyg-colorpicker wysiwyg-fontcolor editorBtn',
                attributes: [{
                    name: 'title',
                    value: 'Font Color'
                }, {
